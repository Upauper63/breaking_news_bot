...
"""
with open('breaking_news_bot.yaml') as f:
...
with open('breaking_news_bot.yaml', 'w') as f:
...
for entry in cont['entries']:
    ...
    if entry_date >= datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=9))) - datetime.timedelta(days=1):
        no_news = False
